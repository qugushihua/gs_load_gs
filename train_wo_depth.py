...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    print("cauculate metrics takes(min):",total_time / 60)
...
def training_report(dataset,tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    # Report test and samples of training set
    if iteration % 100 == 0:
        with open(f'{dataset.model_path}/results.txt', 'a') as f:
            torch.cuda.empty_cache()
            validation_configs = ({'name': 'test', 'cameras' : scene.getTestCameras()}, 
                                  {'name': 'train', 'cameras' : [scene.getTrainCameras()[idx % len(scene.getTrainCameras())] for idx in range(5, 30, 5)]})
            ...
            for config in validation_configs:
                if config['cameras'] and len(config['cameras']) > 0:
                    ...
                    lpips_test = 0.0
                    ssim_test = 0.0
                    gaussians_count = scene.gaussians._xyz.shape[0] # 获取高斯数量
                    for idx, viewpoint in enumerate(config['cameras']):
                        image = torch.clamp(renderFunc(viewpoint, scene.gaussians, *renderArgs)["render"], 0.0, 1.0)
                        gt_image = torch.clamp(viewpoint.original_image.to("cuda"), 0.0, 1.0)
                        if tb_writer and (idx < 5):
                    ...
                    f.write("\n[ITER {}] Evaluating {}: L1 {} PSNR {} SSIM {} LPIPS {} Gaussians Count {}\n".format(iteration, config['name'], l1_test, psnr_test, ssim_test, lpips_test, gaussians_count))
                    if tb_writer:
                        tb_writer.add_scalar(config['name'] + '/loss_viewpoint - l1_loss', l1_test, iteration)
                        tb_writer.add_scalar(config['name'] + '/loss_viewpoint - psnr', psnr_test, iteration)
